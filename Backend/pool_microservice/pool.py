...
class Pool(db.Model):
    ...
    PoolID = db.Column(db.String(36), primary_key=True)
    ...
    UserID = db.Column(db.String(36), nullable=False)
    Status = db.Column(db.String(36), nullable=False)

    def __init__(self, PoolID, DateCreation, Expiry_Date, Current_amount, Budget, Pool_Type, UserID, Status):
        self.PoolID = PoolID
...
@app.route("/Pool/<string:PoolID>", methods=['PUT'])
def update_pool(PoolID):
    ...
    if pool:
        try:
            ...
            return jsonify({"code": 500, "data": data, "message": "An error occurred updating the pool."}), 500
    else:
...
@app.route("/Pool/<string:PoolID>", methods=['GET'])
def get_pool(PoolID):
    pool = Pool.query.filter_by(PoolID=PoolID).first()
    if pool:
        return jsonify({"code": 200, "data": pool.json()})
